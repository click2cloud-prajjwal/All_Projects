...
RICE_PEST_MODEL_PATH = "/mnt/models/Rice_pest_epoch_30.pth"
rice_pest_checkpoint = torch.load(RICE_PEST_MODEL_PATH, map_location="cpu")
efficientnet_rice_pest_model = timm.create_model("efficientnet_b3", pretrained=False, num_classes=6)
efficientnet_rice_pest_model.load_state_dict(rice_pest_checkpoint["model_state_dict"])
efficientnet_rice_pest_model.eval()

rice_pest_class_map = joblib.load("/mnt/models/Rice_pest_class_map.pkl")
rice_pest_index_to_label = {v: k for k, v in rice_pest_class_map.items()}
...
# Load Class Mapping
class_map = joblib.load("/mnt/models/class_map.pkl")
...
def predict_rice_pest(image):
    ...
    with torch.no_grad():
        outputs = efficientnet_rice_pest_model(image_tensor)
        probabilities = torch.nn.functional.softmax(outputs, dim=1).cpu().numpy()[0]

    predicted_class = int(np.argmax(probabilities))
    confidence = float(np.max(probabilities))

    if predicted_class not in rice_pest_index_to_label:
        return f"Unknown Class {predicted_class}"

    label = rice_pest_index_to_label[predicted_class]
    return f"{label}"
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            for image_index, image_file in enumerate(image_files):  # Use image_index to ensure uniqueness
                ...
                if crop_name == "Rice Pest":
                    prediction = predict_rice_pest(image_pil)
