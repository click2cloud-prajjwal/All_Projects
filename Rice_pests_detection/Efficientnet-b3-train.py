...
model = create_model("efficientnet_b3", pretrained=True, num_classes=len(full_dataset.classes))
...
def train(model, train_loader, valid_loader, criterion, optimizer, scheduler, num_epochs=30):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, f"new_checkpoint_epoch_{epoch+1}.pth")
...
# Save Final Model
torch.save(model.state_dict(), "Rice_pest_classifier.pth")
print("Final model saved as 'Rice_pest_classifier.pth'")
